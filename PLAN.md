...
7. Static, DNS A/AAAA/SRV, and file-fed dynamic upstreams before orchestrator integrations.
...
| Hickory resolver | DNS A/AAAA/SRV resolution with TTLs and async support | Active Rust DNS project; Apache-2.0/MIT | system resolver, trust-dns legacy name | Needed for health-aware DNS discovery and revalidation | DNS poisoning/rebinding; explicit resolvers optional, validate every resolved IP against egress policy, cap answers/TTL. |
...
- **Dependencies:** Phase 3 and Hickory for bounded TTL-aware A/AAAA/SRV resolution; the system resolver remains an explicitly documented fallback mode only if its refresh semantics meet the deployment requirement.
