...
| Session fixation/CSRF/admin XSS | default-disabled exact-origin loopback/OIDC configuration; session and UI routes not yet implemented | ADR-0029; ADR-0030; configuration validation and redaction tests | Phase 16 in progress; independent application-security gate before UI ships |
| First-run administrator takeover | setup token requires Admin Unix peer and durable audit, binds exact `uid-<uid>` owner, stores one process-memory SHA-256 digest, expires in ten minutes, and rejects bearer use | setup-token replacement/expiry/replay unit check; CLI bearer-denial, owner, audit, no-store, and secret-canary integration | verified-local for issuance; browser redemption and race review remain Phase 16 work |
...
| Dependency supply chain | lockfiles; exact internal versions; registry-only sources; audit/deny gates | dependency review; `cargo-audit`, `cargo-deny`, source scan | verified-local with RUSTSEC-2026-0173 dated exception |
