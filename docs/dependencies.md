...
| fs2 | Cross-platform exclusive state-directory, Access Policy store, and per-certificate order locks on MSRV 1.88 | default | MIT/Apache-2.0 | Platform locking through libc/Windows APIs; no project unsafe | stale create-new lockfile, newer std lock API above current MSRV | Pin 0.4 lockfile; multi-owner and crash-release tests |
