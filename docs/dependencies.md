...
| axum | Private Unix-socket administrative HTTP API, typed routing, bounded JSON extractors, and graceful shutdown | `http1`, `json`, `tokio`; defaults disabled | MIT | No project unsafe; reuses audited Hyper/Tokio stack | raw Hyper services | Pin 0.8.9 lockfile; API/RBAC/resource-limit tests and advisory gate on upgrade |
