...
pub(crate) fn check(
    ...
    limiters.get(id).ok_or(())?.check(address, Instant::now())
}
...
impl RateLimiter {
    ...
    fn check(&self, address: IpAddr, now: Instant) -> Result<Outcome, ()> {
        let mut buckets = self.buckets.lock().map_err(|_| ())?;
        if !buckets.contains_key(&address) && buckets.len() >= self.max_keys {
...
mod tests {
    ...
    fn token_bucket_refills_and_key_store_stays_bounded() {
        ...
        assert_eq!(limiter.check(first, now), Ok(Outcome::Allowed));
        assert_eq!(limiter.check(first, now), Ok(Outcome::Allowed));
        assert_eq!(
        ...
        assert_eq!(
            limiter.check(first, now + Duration::from_secs(1)),
            Ok(Outcome::Allowed)
        );

        let second = "192.0.2.2".parse().expect("IP");
        let third = "192.0.2.3".parse().expect("IP");
        assert_eq!(limiter.check(second, now), Ok(Outcome::Allowed));
        assert!(matches!(
            limiter.check(third, now),
            Ok(Outcome::Limited { .. })
        ));
        assert_eq!(
            limiter.check(third, now + Duration::from_secs(11)),
            Ok(Outcome::Allowed)
        );
